...
import { DriveSignal, DriveType, EngineState, DesireEngineConfig } from '../types.js';
 ...
 */
export function generateDrives(
  state: EngineState,
  ...
  if (state.idle_duration_seconds > 0) {
    const anxietyIntensity = computeIdleAnxiety(
      state.idle_duration_seconds,
      config.drives.idle_anxiety_rate,
...
// ============================================================
// Context types for drive generation
// ============================================================
...
export interface DriveContext {
  ...
  lastReflectionAge?: number; // Seconds since last reflection

  // === Phase 2.5: Findings from remote executors ===
  ...
  conversationActivity?: {
    totalLast24h: number;           // Total conversations in last 24h
    incomingLast24h: number;        // Incoming messages in last 24h
    outgoingLast24h: number;        // Outgoing messages in last 24h
    unansweredIncoming: number;     // Incoming messages with no subsequent outgoing
    channelBreakdown: Record<string, number>; // Messages per channel
  };

  /** Owner presence and interaction recency */
  ownerPresence?: {
    lastInteractionAgeSeconds: number; // Seconds since last owner message
    ownerActiveToday: boolean;          // Owner sent at least 1 message today
    recentOwnerMessages: number;        // Owner messages in last 6h
  };

  /** Scheduler task status from C5 */
  schedulerStatus?: {
    pendingTasks: number;              // Tasks waiting to run
    failedRecent: number;              // Failed tasks in last 48h
    successRecent: number;             // Succeeded tasks in last 48h
    successRate: number;               // success / (success + failed) in last 48h
    overdueCount: number;              // Tasks past their scheduled time
    runningTasks: number;              // Currently executing tasks
  };
}
