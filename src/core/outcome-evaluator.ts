...
export function evaluateOutcome(
  ...
  const learningRate = computeDynamicLearningRate(
    Math.abs(rpe),
    config.learning.rpe_learning_rate,
    config.learning.min_learning_rate,
    config.learning.max_learning_rate
  );

  // Update predicted_reward (exponential moving average)
  desire.predicted_reward = clamp(
    desire.predicted_reward + learningRate * rpe,
    0, 1
  );

  // Update wanting_score based on RPE
  desire.wanting_score = clamp(
    desire.wanting_score + config.learning.reinforcement_rate * rpe,
    0, config.safety.max_wanting_score  // Ceiling prevents addiction
  );
  ...
  desire.novelty_bonus = clamp(
    desire.novelty_bonus * Math.exp(-decayRate * ageHours),
    0.1, 1 // Floor 0.1 to avoid zero-out in salience calculation
  );
  ...
  if (Math.abs(rpe) < 0.1) {
    ...
    desire.habit_strength = clamp(
      desire.habit_strength + config.learning.habituation_rate,
      0, 1
    );
  } else if (rpe < -0.3) {
    // Large negative RPE → break habit (re-engage goal-directed mode)
    desire.habit_strength = clamp(
      desire.habit_strength - config.learning.habituation_rate * 3,
      0, 1
    );
  }
  ...
  state.baseline_motivation = clamp(
    state.baseline_motivation + rpe * 0.05,
    0.2, 1.0  // Floor prevents depression-like state
  );
  ...
  if (state.baseline_motivation < 0.35) {
    state.baseline_motivation = clamp(
      state.baseline_motivation + 0.01, // Slow auto-recovery per cycle
      0.2, 1.0
    );
  }
  state.recent_success_rate = clamp(
    state.recent_success_rate * 0.9 + (actualReward > 0.5 ? 0.1 : 0),
    0, 1
  );
 ...
 */
function detectWantingLikingDivergence(desire: Desire): string | null {
 ...
 */
function computeDynamicLearningRate(
  absRpe: number,
  baseLR: number,
  minLR: number,
  maxLR: number
