...
import { Desire, DesireEngineConfig, EngineState, DriveType } from '../types.js';
...
export function runSafetyChecks(
  ...
  const wantingMax = Math.min(
    config.safety.max_wanting_score,
    SAFETY_INVARIANTS.WANTING_ABSOLUTE_MAX
  );
  const addictionGap = SAFETY_INVARIANTS.ADDICTION_DIVERGENCE_THRESHOLD;
  const obsessionMax = Math.min(
    config.safety.max_active_desires > 0 ? 20 : 20, // config might have its own
  ...
  const activeDriveTypes = new Set<string>();
  for (const d of activeDesires) {
  ...
  const maxDesires = Math.min(
    config.safety.max_active_desires,
    SAFETY_INVARIANTS.MAX_ACTIVE_DESIRES_HARD_LIMIT
  );
...
function getTopDriveType(desire: Desire): string {
