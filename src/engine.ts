...
import { DesireEngineConfig, Desire, EngineState, EngineMode, DriveSignal, OutcomeRecord, RewardSource, RiskLevel, DriveType, ExecutorInfo, ExecutorMetrics, DesireOutcomeMessage } from './types.js';
import { generateDrives, DriveContext } from './core/drive-generator.js';
import { computeMotivationScore, rankDesires } from './core/salience-calculator.js';
import { planNextAction, PlanDecision } from './core/executive-planner.js';
import { evaluateOutcome } from './core/outcome-evaluator.js';
import { runSafetyChecks, SafetyReport, SAFETY_INVARIANTS } from './core/safety.js';
import { DesireStore } from './store/desire-store.js';
import { VoiceGenerator, VoiceMessage } from './core/voice.js';
...
export type EngineEvent =
  | { type: 'cycle_complete'; decision: PlanDecision; safety: SafetyReport; cycle: number }
  | { type: 'desire_created'; desire: Desire }
  | { type: 'desire_completed'; desire: Desire; outcome: OutcomeRecord }
  | { type: 'desire_abandoned'; desire: Desire; reason: string }
  | { type: 'mode_changed'; from: EngineMode; to: EngineMode }
  | { type: 'approval_needed'; desire: Desire; score: number }
  | { type: 'safety_alert'; report: SafetyReport }
  | { type: 'explore'; signals: DriveSignal[] }
  | { type: 'reflect'; state: EngineState }
  | { type: 'voice'; message: VoiceMessage };
...
export class DesireEngine {
  private store: DesireStore;
  private config: DesireEngineConfig;
  ...
  private voice: VoiceGenerator;
  private executorRegistry: Map<string, ExecutorInfo> = new Map();
   ...
   */
  async runCycle(): Promise<PlanDecision> {
      ...
      case 'pursue': {
        ...
        try {
          ...
          const outcome = evaluateOutcome(
            decision.desire,
            0,
            RewardSource.COMPLETION,
            `Hard failure: ${(err.message || 'unknown error').slice(0, 200)}`,
            this.config,
            state
          );
          this.store.addOutcome(outcome);
   ...
   */
  getDesireRanking(): Array<{ desire: Desire; score: number; probability: number }> {
    const state = this.store.getState();
  ...
  updateConfig(updates: Partial<DesireEngineConfig>): void {
    ...
    if (violations.length > 0) {
      throw new Error(
        `[DesireEngine] Cannot modify frozen config keys: ${violations.join(', ')}. ` +
    ...
    if (updates.safety) {
      if (updates.safety.max_wanting_score !== undefined &&
        ...
        throw new Error(
          `[DesireEngine] max_wanting_score cannot exceed invariant ${SAFETY_INVARIANTS.WANTING_ABSOLUTE_MAX}`
        );
      }
    }
    ...
    if (updates.drives?.action_threshold !== undefined &&
      ...
      throw new Error(
        `[DesireEngine] action_threshold cannot be below invariant ${SAFETY_INVARIANTS.ACTION_THRESHOLD_FLOOR}`
      );
    }
  ...
  handleRemoteOutcome(msg: DesireOutcomeMessage): void {
    ...
    if (execMetrics.liking_history.length > 10) execMetrics.liking_history.shift();
    ...
    const state = this.store.getState();
    const outcome = evaluateOutcome(
      desire, msg.reward, msg.reward_source, msg.action_description,
      this.config, state
    );
  ...
  private async emit(event: EngineEvent): Promise<void> {
    ...
    if (event.type !== 'voice') {
      const voiceMsg = this.voice.generate(
        event,
        this.store.getAllDesires(),
        this.store.getState()
      );
      if (voiceMsg) {
  ...
  expressDesires(): VoiceMessage | null {
    return this.voice.expressCurrentDesires(
      this.store.getAllDesires(),
      this.store.getState()
    );
  }
...
function checkFrozenKeys(updates: Partial<DesireEngineConfig>, prefix = ''): string[] {
  ...
  for (const [key, value] of Object.entries(updates)) {
    ...
    if (FROZEN_CONFIG_PATHS.has(fullPath)) {
      ...
      violations.push(...checkFrozenKeys(value as any, fullPath));
    }
  }
  return violations;
}
