...
import { DesireEngineConfig, Desire, EngineMode, RewardSource, DriveType, RiskLevel, ExecutorInfo, DesireTaskMessage, DesireOutcomeMessage } from './types.js';
...
export function loadConfig(configPath?: string): DesireEngineConfig {
  const p = configPath || CONFIG_PATH;
  const raw = fs.readFileSync(p, 'utf-8');
  return yaml.parse(raw) as DesireEngineConfig;
}
...
export function createZylosContextProvider(): ContextProvider {
  return async (): Promise<DriveContext> => {
    ...
    try {
      const staleDomains: DriveContext['staleDomains'] = [];
      const referenceDir = path.join(MEMORY_DIR, 'reference');

      if (fs.existsSync(referenceDir)) {
        const files = fs.readdirSync(referenceDir).filter((f: string) => f.endsWith('.md'));
        const now = Date.now();
    ...
    try {
      if (fs.existsSync(FINDINGS_PATH)) {
        const allFindings: FindingsEntry[] = JSON.parse(fs.readFileSync(FINDINGS_PATH, 'utf-8'));
        const grouped: Record<string, Array<{ summary: string; executor_id: string; timestamp: string }>> = {};

        // Group last 20 findings by desire_id
        for (const f of allFindings.slice(-20)) {
          if (!grouped[f.desire_id]) grouped[f.desire_id] = [];
...
function queryC4(sql: string): string {
  const fullSql = `PRAGMA busy_timeout=5000; ${sql}`;
  const raw = execSync(
    `sqlite3 "${C4_DB}" "${fullSql}"`,
...
function getConversationActivity(): DriveContext['conversationActivity'] {
  if (!fs.existsSync(C4_DB)) return undefined;

  const result = queryC4(
    `SELECT direction, channel, COUNT(*) as cnt ` +
    `FROM conversations ` +
  ...
  for (const line of result.split('\n').filter(Boolean)) {
    ...
    totalLast24h += cnt;
    if (direction === 'in') incomingLast24h += cnt;
    if (direction === 'out') outgoingLast24h += cnt;
  ...
  const unansweredResult = queryC4(
    `SELECT COUNT(*) FROM conversations c1 ` +
...
function getOwnerPresence(): DriveContext['ownerPresence'] {
  if (!fs.existsSync(C4_DB)) return undefined;

  const lastOwnerMsg = queryC4(
  ...
  const ownerTodayResult = queryC4(
...
function getSchedulerStatus(): DriveContext['schedulerStatus'] {
  // Parse scheduler list output for pending/failed counts
  const listRaw = execSync(
    `node "${SCHEDULER_CLI}" list --json 2>/dev/null || echo "[]"`,
    { encoding: 'utf-8', timeout: 5000 }
  ).trim();
  ...
  const historyRaw = execSync(
    `node "${SCHEDULER_CLI}" history 2>/dev/null || echo ""`,
    { encoding: 'utf-8', timeout: 5000 }
  ).trim();
  ...
  for (const line of historyLines) {
    ...
    if (dateMatch) {
      try {
        ...
        if (entryDate < sevenDaysAgo) continue; // Skip old entries
      } catch { continue; }
    }
    if (/success/i.test(line)) successRecent++;
    if (/failed|timeout/i.test(line)) failedRecent++;
  }
...
function loadRewardHistory(): RewardHistory {
  try {
...
function saveRewardHistory(history: RewardHistory): void {
  try {
...
function recordReward(driveType: string, reward: number, detail: string): void {
 ...
 */
function getHistoricalReward(driveType: string, maxEntries = 5): number | undefined {
  const history = loadRewardHistory();
...
export function createZylosActionExecutor(opts?: {
  ...
  const recentlyScheduled = new Map<string, number>(); // desire_id → timestamp
  ...
  return async (desire: Desire, decision: PlanDecision): Promise<ActionResult> => {
...
async function executeCognitiveAction(desire: Desire): Promise<ActionResult> {
  ...
  try {
    ...
    execSync(
      `node "${SCHEDULER_CLI}" add "${escapeShell(taskDescription)}" --in 1m`,
      { encoding: 'utf-8' }
    );
    ...
    try {
      ...
      const researchLines = historyRaw.split('\n').filter(l =>
        /Desire Engine.*Research/i.test(l) || /Research/i.test(l)
      );
...
async function executeSocialAction(desire: Desire): Promise<ActionResult> {
  ...
  try {
    ...
    execSync(
      `node "${C4_SEND}" "telegram" "6471698262" "${escapeShell(message)}"`,
      { encoding: 'utf-8' }
    );
      ...
      if (fs.existsSync(C4_DB)) {
        ...
        if (shareMessages) {
          ...
          for (const shareLine of shares) {
            ...
            if (!shareId || !shareTs) continue;

            // Check if owner replied within 24h of this share
            const replyCheck = queryC4(
              `SELECT timestamp FROM conversations ` +
              `WHERE direction = 'in' ` +
              `AND endpoint_id = '${OWNER_ENDPOINT}' ` +
              `AND id > ${shareId} ` +
...
async function executeHomeostaticAction(desire: Desire): Promise<ActionResult> {
  ...
  try {
    ...
    try {
      ...
      const currentDisk = parseInt(
        execSync("df -h / | tail -1 | awk '{print $5}'", { encoding: 'utf-8' }).trim().replace('%', ''),
        10
      ) || 0;
...
async function executeSelfActAction(desire: Desire): Promise<ActionResult> {
 ...
 */
export async function dispatchToRemoteExecutor(
  desire: Desire,
  executor: ExecutorInfo
): Promise<ActionResult> {
  const message: DesireTaskMessage = {
    type: 'desire-task',
    desire_id: desire.id,
    goal: desire.goal,
  ...
  try {
    execSync(
      `node "${C4_SEND}" "hxa-connect" "${executor.c4_endpoint}" "${escapeShell(messageJson)}"`,
 ...
 */
export function storeFindingsSummary(desireId: string, executorId: string, summary: string): void {
  try {
    let findings: FindingsEntry[] = [];
    if (fs.existsSync(FINDINGS_PATH)) {
 ...
 */
export function getRecentFindings(desireId: string, limit = 5): FindingsEntry[] {
  try {
    if (!fs.existsSync(FINDINGS_PATH)) return [];
    const findings: FindingsEntry[] = JSON.parse(fs.readFileSync(FINDINGS_PATH, 'utf-8'));
    return findings
...
export function createZylosEventHandler(): (event: EngineEvent) => void {
  const logFile = path.join(DATA_DIR, 'engine.log');
...
export function createZylosDesireEngine(opts?: {
  ...
  let engineRef: DesireEngine | null = null;
  ...
  const engine = new DesireEngine({
    ...
    actionExecutor: createZylosActionExecutor({
      getRemoteExecutor: (desire: Desire) => {
...
function truncateReplacer(_key: string, value: any): any {
