...
import { EngineMode, DesireOutcomeMessage, RewardSource } from './types.js';
...
let lastCheckedId = 0;
const processedOutcomeIds = new Set<number>();

function pollForOutcomes(): void {
  try {
    if (!fs.existsSync(C4_DB)) return;
    ...
    const dataLines = lines.filter((l: string) => l.includes('|') && !(/^\d+$/.test(l)));
    ...
    for (const line of dataLines) {
      ...
      if (processedOutcomeIds.has(msgId)) continue;
 ...
 */
function extractOutcomeMessage(content: string): DesireOutcomeMessage | null {
...
try {
  if (fs.existsSync(C4_DB)) {
    const maxId = execSync(
      `sqlite3 "${C4_DB}" "SELECT COALESCE(MAX(id), 0) FROM conversations"`,
      { encoding: 'utf-8', timeout: 5000 }
    ).trim();
