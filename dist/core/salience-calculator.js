 ...
 */
export function computeMotivationScore(desire, config, state) {
    ...
    const value = desire.wanting_score * 0.30 +
 ...
 */
export function rankDesires(desires, config, state, temperature = 0.3) {
    if (desires.length === 0)
        return [];
    // Compute raw scores
 ...
 */
function computeTemporalDiscount(deadline, discountRate, urgencyBoost) {
    const now = Date.now();
