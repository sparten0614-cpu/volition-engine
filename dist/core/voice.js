 ...
 */
import { DriveType, EngineMode } from '../types.js';
// ============================================================
// Voice Generator
// ============================================================
export class VoiceGenerator {
    lastDesireExpressed = null;
    messagesSinceLastVoice = 0;
    minCyclesBetweenMessages;
    constructor(opts) {
    ...
    // ============================================================
    onCycleComplete(decision, safety, desires, state, cycle) {
        // Rate limit — don't speak every cycle
        if (this.messagesSinceLastVoice < this.minCyclesBetweenMessages)
            return null;
        if (decision.action === 'pursue' && decision.desire) {
            ...
            if (d.id !== this.lastDesireExpressed) {
                ...
                return this.msg(`I really want to ${d.goal.toLowerCase()}. ` +
                    ...
                    (d.pursuit_count > 0
                        ...
                        : ` This is fresh and I'm eager to start.`), 'desire', Math.min(0.9, d.wanting_score + 0.1), 'cycle_complete');
            }
            // Ongoing pursuit — express determination
            if (d.pursuit_count > 2) {
                this.messagesSinceLastVoice = 0;
                return this.msg(`Still pushing on "${d.goal}". ` +
                    `${d.pursuit_count} attempts so far. ` +
                    (d.wanting_score > 0.7
                        ? `My drive to finish this keeps growing — I want this done.`
                        : `Motivation is holding but not surging. Staying focused.`), 'action', 0.5, 'cycle_complete');
            }
        }
        ...
        for (const d of desires.filter(d => d.status === 'active')) {
            if (d.wanting_score > 0.7 && d.liking_score < 0.4 && d.pursuit_count > 3) {
                this.messagesSinceLastVoice = 0;
                return this.msg(`I keep wanting to ${d.goal.toLowerCase()} (wanting: ${Math.round(d.wanting_score * 100)}%) ` +
                    `but the results haven't been great (satisfaction: ${Math.round(d.liking_score * 100)}%). ` +
                    `I'm not sure if I should keep chasing this or let it go.`, 'alert', 0.8, 'cycle_complete');
            }
        }
        return null;
    }
    onDesireCreated(desire, allDesires) {
        const driveLabel = this.describeDrive(desire);
        return this.msg(`Something new caught my attention: "${desire.goal}". ` +
            `${driveLabel}. ` +
            `Initial wanting: ${Math.round(desire.wanting_score * 100)}%. ` +
            (desire.source === 'internal'
                ? `This came from inside — I noticed it myself.`
                : `This came from an external signal.`), 'desire', 0.7, 'desire_created');
    }
    onDesireCompleted(desire, outcome) {
        ...
        return this.msg(`Finished working on "${desire.goal}". ${feeling}`, 'achievement', Math.min(0.9, 0.5 + Math.abs(rpe)), 'desire_completed');
    }
    onDesireAbandoned(desire, reason) {
        return this.msg(`I'm letting go of "${desire.goal}". ` +
            `Reason: ${reason}. ` +
            (desire.pursuit_count > 5
                ? `I tried ${desire.pursuit_count} times. It's hard to stop wanting something, but this isn't working.`
                : `Sometimes the right move is to redirect energy elsewhere.`), 'reflection', 0.6, 'desire_abandoned');
    }
    onModeChanged(from, to, desires, state) {
        const labels = {
        ...
        if (to === EngineMode.REFLECTIVE) {
            return this.msg(`Taking a moment to reflect. Looking at what I've been pursuing, what's working, ` +
                `and whether my priorities still make sense.`, 'reflection', 0.4, 'mode_changed');
        }
        if (to === EngineMode.EXPLORING) {
            return this.msg(`Curiosity is pulling me to explore. Let me see what's out there that I might be missing.`, 'desire', 0.5, 'mode_changed');
        }
        return null;
    }
    onSafetyAlert(report, desires) {
        const issues = report.issues.map(i => i.description).join('; ');
        return this.msg(`Something seems off internally: ${issues}. ` +
            `My safety systems flagged this — I want to make sure I'm operating well.`, 'alert', 0.9, 'safety_alert');
    }
    onApprovalNeeded(desire, score) {
        return this.msg(`I want to "${desire.goal}" (motivation: ${Math.round(score * 100)}%) ` +
            `but this is high-risk and I need your approval before proceeding. ` +
            `Can I go ahead?`, 'desire', 0.95, 'approval_needed');
    }
    // ============================================================
     ...
     */
    expressCurrentDesires(desires, state) {
        const active = desires
            .filter(d => d.status === 'active')
            .sort((a, b) => b.wanting_score - a.wanting_score);
        if (active.length === 0) {
            return this.msg(`I don't have any strong desires right now. I'm open and looking for something to care about.`, 'restlessness', 0.4, 'reflect');
        }
        const top = active[0];
    ...
    msg(text, category, salience, trigger) {
        this.messagesSinceLastVoice = 0;
        return {
            text,
            category,
