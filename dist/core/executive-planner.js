 ...
 */
import { EngineMode, RiskLevel } from '../types.js';
 ...
 */
export function planNextAction(desires, signals, config, state) {
    ...
    const effectiveThreshold = Math.max(config.drives.action_threshold, SAFETY_INVARIANTS.ACTION_THRESHOLD_FLOOR);
    // Rank desires
 ...
 */
function shouldReflect(state, config, signals) {
    ...
    if (state.mode === EngineMode.REFLECTIVE)
        return false;
 ...
 */
function shouldExplore(state, config, signals) {
    const curiositySignal = signals.find(s => s.source === 'curiosity_gap' || s.source === 'idle_anxiety');
 ...
 */
function checkRiskGate(desire, config) {
